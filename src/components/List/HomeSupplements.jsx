...
useEffect(() => {
   axiosUrl.get('/home')  // Laravel API endpoint for fetching categories
          .then((response) => {
           ...
           setSupplements(response.data.supplies_listings);
           //    setLoading(false);
          })
          .catch((error) => {
    ...
    <section class="ruk-dogs-list-sec">
        <div class="ruk-bully-container">
            <div class="dog-listing-heading remo-ex-space">
                <h2 class="text-uppercase fw-bold">Food And Supplements </h2>
              ...
              <div class="dog-list-col">    
                  <div class="dog-card">
                      <a href="#" class="box-link">
                          <img src={`http://127.0.0.1:8000${supplement.gallery1}`} alt="American" class="img-fluid rounded" />
                          <span class="heart-icon">
                          </span></a><a class="favourite-button">
                          <i class="fa fa-heart-o" aria-hidden="true"></i>
                      </a>
                      <div class="dog-card-body">
                          <h3 class="dog-title scape-line">{supplement.title}</h3>
                          <p class="dog-category scape-line">Category: {supplement.categoryName}</p>
                          <p class="dog-price">${supplement.price}</p>
                      </div>
                  </div> 
              </div>
              ))
...
export default HomeSupplements
